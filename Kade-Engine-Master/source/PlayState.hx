...
class PlayState extends MusicBeatState
{
			...
			case 'trucks':
				{
						...
						var bg:FlxSprite = new FlxSprite().loadGraphic(Paths.image('trucksBG'));
						bg.scrollFactor.set(1, 1);
						add(bg);

						freesmart = new FreesmartDance(489.9, 190.15);
					    freesmart.scrollFactor.set(1, 1);
						add(freesmart);

						var stage:FlxSprite = new FlxSprite().loadGraphic(Paths.image('trucksStage'));
						stage.scrollFactor.set(1, 1);
						add(stage);

						flower = new FlxSprite(1060.1, 588.8).loadGraphic(Paths.image('flowerfront'));
						flower.scrollFactor.set(1, 1);
				}
			case 'stage':
				{
						defaultCamZoom = 0.9;
	...
	function startCountdown():Void
	{
		...
		blackIntro = new FlxSprite(-100, -100).makeGraphic(FlxG.width * 2, FlxG.height * 2, FlxColor.BLACK);
		blackIntro.scrollFactor.set();
		if (curSong == 'Two Trucks')
		{
			add(blackIntro);
		}
	...
	function startSong():Void
	{
		...
		{	
			...
			new FlxTimer().start(225.66, function(swagTimer:FlxTimer)
			{
				blackIntro.alpha += .1;
				new FlxTimer().start(2.1, function(swagTimer:FlxTimer)
				{
					...
					{
						new FlxTimer().start(2.24, function(swagTimer:FlxTimer)
						{
							blackIntro.alpha += .1;
							new FlxTimer().start(1.96, function(swagTimer:FlxTimer)
							{
								blackIntro.alpha += .1;
								new FlxTimer().start(2.1, function(swagTimer:FlxTimer)
								{
									blackIntro.alpha += .1;
									if (blackIntro.alpha < .8)
									{
										swagTimer.reset();
									}
									else
	...
	private function generateStaticArrows(player:Int):Void
	{
		...
		{
			// FlxG.log.add(i);
			var babyArrow:FlxSprite = new FlxSprite(375, strumLine.y);
	...
	override function stepHit()
	{
		...
		if (curSong == 'Two Trucks')
		{
			switch (curStep)
			{
				...
				case 1202:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf-jump');
					add(boyfriend);
				case 1214:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf');
					add(boyfriend);
			}
		}
	...
	override function beatHit()
	{
		...
		if (curSong == 'Two Trucks')
		{
			switch (curBeat)
			{
				case 167:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf-christmas');
					add(boyfriend);
				case 183:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf-pixel');
					add(boyfriend);
				case 198:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf-car');
					add(boyfriend);
				case 226:
					remove(boyfriend);
					boyfriend = new Boyfriend(805, 480, 'bf');
